# To Identify the Probability of H1B application being CERTIFIED with respect to PREVAILING WAGE and WORKSITE STATE

library(ggplot2)
# Read the data from csv file
mydata = read.csv("finaldata.csv",header = TRUE)
...
newdata <- subset(mydata, (CASE_STATUS == "CERTIFIED" | CASE_STATUS == "DENIED") 
                  & PW_UNIT_OF_PAY == "Year" & YEAR == "2014" & PREVAILING_WAGE != "NA",
...
mylogit <- glm(CASE_STATUS ~ PREVAILING_WAGE + WORKSITE_STATE, data = newdata, family = "binomial")
summary(mylogit)
# confint(mylogit)
exp(coef(mylogit))
...
newdata2 <- with(subset(newdata,PREVAILING_WAGE < 400000), data.frame(PREVAILING_WAGE = rep(seq(from = min(PREVAILING_WAGE),
      to = max(PREVAILING_WAGE),length.out = 100),57),WORKSITE_STATE = factor(rep(unique(WORKSITE_STATE), each = 100))))
...
ggplot(subset(newdata3,WORKSITE_STATE == "IL" | WORKSITE_STATE == "VI" | WORKSITE_STATE == "DE"), aes(x = PREVAILING_WAGE, 
               y = PredictedProb))+geom_ribbon(data = subset(newdata3,WORKSITE_STATE == "IL" 
              | WORKSITE_STATE == "VI" | WORKSITE_STATE == "DE"), aes(ymin = LL, ymax = UL, 
               fill = WORKSITE_STATE), alpha = 0.2) + geom_line(aes(colour = WORKSITE_STATE),size = 1)
#ggplot(newdata3, aes(x = PREVAILING_WAGE, y = PredictedProb))+geom_ribbon(data = newdata3, aes(ymin = LL, ymax = UL, fill = WORKSITE_STATE), alpha = 0.2) + geom_line(aes(colour = WORKSITE_STATE),size = 1)
